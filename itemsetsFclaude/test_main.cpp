...
int main() {
  ...
  cout << "Size wt = " << wt->getSize() << " - "
       << (wt->getSize() * 1.0) / (1024 * 1024.0) << endl;
  cout << "Size using RL array " << size_a << " - "
       << (1.0 * size_a) / (1024 * 1024.0) << endl;
  cout << "Size using RL Bitmap " << size_b << " - "
       << (1.0 * size_b) / (1024 * 1024.0) << endl;
  size_t si, total = 0;
  size_t total_len = 0;
  size_t i = 0;
  int m = 0;
  ...
  cout << "Labels: " << m << " " << bits(m) * total_len / 8 << " "
       << 1.2 * bits(m) * total_len / 8.0 << endl;
}
